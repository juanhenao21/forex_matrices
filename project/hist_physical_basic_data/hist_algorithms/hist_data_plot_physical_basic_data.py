...
def hist_fx_returns_year_plot(fx_pair: str, year: str,
                              weeks: Tuple[str, ...]) -> None:
    """Plots the returns for a year.
    ...
    function_name: str = hist_fx_returns_year_plot.__name__
    hist_data_tools_physical_basic_data \
        .hist_function_header_print_plot(function_name, fx_pair, year, '')
    fx_pair_upper: str = fx_pair[:3].upper() + '/' + fx_pair[4:].upper()
    ...
    for week in weeks:
        ...
        plt.plot(fx_data['Returns'], 'g', linewidth=5)

    plt.title(f'HIST returns - {fx_pair_upper}', fontsize=40)
    plt.xlabel(r'Time $[s]$', fontsize=35)
    plt.ylabel(r'Returns', fontsize=35)
    plt.xticks(fontsize=25)
    plt.yticks(fontsize=25)
    plt.grid(True)
    plt.tight_layout()
